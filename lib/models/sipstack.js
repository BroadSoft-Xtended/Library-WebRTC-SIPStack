module.exports = require('bdsft-sdk-model')(SIPStack, {
...
function SIPStack(eventbus, debug, urlconfig, cookieconfig) {
  ...
  self.updateReceiveVideo = function(audioOnlyView){
    self.receiveVideo = !audioOnlyView;
  };

  self.updateSendVideo = function(audioOnly){
    self.sendVideo = !audioOnly;
  };
  ...
  self.updateUserMedia = function(value, name, userMediaCallback, failureCallback, changeSessionOptions) {
    if (!self.ua) {
      return;
    }
    ...
    if (self.enabled && (self.enableConnectLocalMedia || self.activeSession)) {
      ...
      self.ua.getUserMedia(options, function(localStream) {
        ...
        if (self.activeSession && !isWaitingForAnswer) {
          ...
          changeSessionOptions = changeSessionOptions || {};
          changeSessionOptions.localMedia = localStream;
          changeSessionOptions.createOfferConstraints = options.createOfferConstraints;
          self.activeSession.changeSession(changeSessionOptions, function() {
  ...
  self.bindings = {
    mediaConstraints: {
      sipstack: ['sendAudio', 'sendVideo', 'encodingResolution', 'hd']
    },
    uaConfig: {
    ...
    userMedia: {
      sipstack: ['sendAudio', 'sendVideo', 'encodingResolution', 'enableConnectLocalMedia', 'receiveVideo']
    },
    bandwidth: {
  ...
  self.endCall = function(options) {
    ...
    if(!self.enableConnectLocalMedia && self.ua && self.ua.localMedia) {
      ...
      self.ua.localMedia.getTracks().forEach(function(track){
        track.stop();
      });
    }
  };
  ...
  self.hold = function(successCallback, failureCallback) {
    if (self.activeSession) {
      self.updateUserMedia(null, null, successCallback, failureCallback, {
        hold: true
      });
    }
  };
    ...
    var options = {
      ...
      createOfferConstraints: {
        mandatory: {
          ...
          OfferToReceiveVideo: self.receiveVideo
        }
      }
    };
    return options;
